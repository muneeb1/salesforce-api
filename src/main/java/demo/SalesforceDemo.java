...
public class SalesforceDemo {
	...
	static final String username = "muneeb@a.com";
	static final String password = "yourPasswordWithSecurityToken";
	...
	// Query Opportunities using REST HttpGet
	public static void getOpportunities() {
		System.out.println("\n_______________ Opportunity QUERY _______________");
		try {

			// Set up the HTTP objects needed to make the request.
			HttpClient httpClient = HttpClientBuilder.create().build();

			String uri = baseUri + "/query?q=Select+Id,Name+From+Opportunity";
			...
			if (statusCode == 200) {
				...
				try {
					...
					for (int i = 0; i < j.length(); i++) {
						System.out.println("Opportunity record is: " + i + ". "
								+ json.getJSONArray("records").getJSONObject(i).getString("Id") + " "
								+ json.getJSONArray("records").getJSONObject(i).getString("Name"));
					}
